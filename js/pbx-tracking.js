...
(function () {
  ...
  // ── Session Recording (server + localStorage fallback) ──
  var STORE_KEY = 'pbx_analytics_sessions';
  var MAX_SESSIONS = 50;
  var sessionEvents = [];

  function getServerUrl() {
    var cfg = window.PBXAnalyticsConfig || {};
    return cfg.saveUrl || '/wp-admin/admin-ajax.php?action=pbx_analytics_save';
  }
  ...
  function saveSession() {
    ...
    // 1. Send to server via beacon (reliable on page unload)
    ...
    // 2. Also save to localStorage as fallback (for local dev / offline)
    try {
      ...
      var sessionObj = { id: sid, pageName: PAGE, startedAt: sessionStart, events: sessionEvents };
      if (existing > -1) sessions[existing] = sessionObj;
